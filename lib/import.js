...
export default async function importLoader(module) {
    ...
    //         workerData: module
    //     })
    //     const NIKOLSITAWorker = () => worker.terminate().catch(() => { })
    //     worker.once('message', (msg) => (NIKOLSITAWorker(), console.log(msg.data), resolve(msg)))
    //     worker.once('error', (error) => (NIKOLSITAWorker(), reject(error)))
    // })
